...
bool LLInventoryModel::isInSuitcase(const LLInventoryCategory * cat) const
{
	...
	if(cat->getPreferredType() == LLFolderType::FT_SUITCASE)
		return true;
    while (cat->getParentUUID().notNull())
    {
		...
		if(parent_cat->getPreferredType() == LLFolderType::FT_SUITCASE)
		{
			return true;
		}
		cat = parent_cat;
	...
	for(i = 0; i < count; ++i)
	{
		...
		if(catsp &&
		   (!gIsInSecondLife || (cat->getParentUUID().notNull() || 
			cat->getPreferredType() == LLFolderType::FT_ROOT_INVENTORY ))) // We are in OpenSim
		{
			catsp->push_back(cat);
		}
		else if(catsp &&
...
LLPointer<LLInventoryValidationInfo> LLInventoryModel::validate() const
{
	...
	for(cat_map_t::const_iterator cit = mCategoryMap.begin(); cit != mCategoryMap.end(); ++cit)
	{
		...
		if (isInSuitcase(cat) && !gIsInSecondLife)
		{
