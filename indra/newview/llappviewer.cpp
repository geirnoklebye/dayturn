...
// <FS>
	...
	LLFile clear_file ;
	mPurgeSettings = (clear_file.isfile(clear_settings_filename));
	LLFile clear_color_file;
	bool clear_colors = (clear_color_file.isfile(clear_color_settings_filename));
	...
	if (!success)
	{
        ...
        if (gDirUtilp->fileExists(settings_file_list))
        {
            LL_ERRS() << "Cannot load default configuration file settings_files.xml. "
                << "Please reinstall viewer from https://www.dayturn.com/viewer/index.php?resources/ "
                << "and contact Dayturn support if issue persists after reinstall."
                << LL_ENDL;
        }
        else
        {
